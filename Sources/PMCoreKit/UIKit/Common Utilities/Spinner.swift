...
public class Spinner {
     ...
     */
    public static func ShowLoading(_ message: String = "Loading...",_ style:ActivityIndicatorStyle = .defaultSpinner) {
...
public class SKActivityIndicatorStyle: NSObject {
    ...
    func createDefaultActivityIndicator(in layer: CALayer, size: CGSize, color: UIColor) {
        
        let x = (layer.bounds.size.width - size.width) / 2
        let y = (layer.bounds.size.height - size.height) / 2
        let activityFrame = CGRect(x: x, y: y, width: size.width, height: size.height)
        activityView = UIActivityIndicatorView(frame: activityFrame)
        activityView.style = .whiteLarge
        activityView.color = color
        activityView.startAnimating()
        
        layer.addSublayer(activityView.layer)

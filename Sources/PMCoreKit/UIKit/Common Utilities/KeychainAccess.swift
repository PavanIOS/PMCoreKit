...
class KeychainAccess {
    ...
    func getUUID() -> String? {
        ...
        let bundleIdentifier =  Bundle.main.bundleIdentifier ?? "com.haicinema.haicinema"
        // this is the key we'll use to store the uuid in the keychain
        let uuidKey = "\(bundleIdentifier).unique_uuid"

        // check if we already have a uuid stored, if so return it
        if let uuid = try? self.queryKeychainData(itemKey: uuidKey) {
            return uuid
        }
        ...
        try? self.addKeychainData(itemKey: uuidKey, itemValue: newId)

...
extension JSON {
    ...
    private func getString(_ key:String,_ jsonString:JSON) ->String {
        var returnStr = ""
        if jsonString[key].exists() {
            if jsonString[key].type == Type.number {
                returnStr = jsonString[key].stringValue
            }else if jsonString[key].type == Type.string {
                returnStr = jsonString[key].string ?? ""
            }else if jsonString[key].type == Type.bool {
                if(jsonString[key].bool != nil && jsonString[key].bool!){
                    returnStr = "true"
                }else{
                    returnStr = "false"
                }
            }else if jsonString[key].type == Type.array {
                returnStr = jsonString[key].description
            }else if jsonString[key].type == Type.dictionary {
                returnStr = jsonString[key].description
            }
        }
        let trimmed = returnStr.trimmingCharacters(in: .whitespacesAndNewlines)
        return trimmed
    }
    
    private func convertDate(_ serverDate:String) -> String{
        var formattedDate = serverDate
        if let date = serverDate.toDate("yyyy-MM-dd'T'HH:mm:ss") {
            formattedDate = date.toString(DateFormatsList.Parser_Date)
        }else if let date = serverDate.toDate("yyyy-MM-dd'T'HH:mm:ss.SSS") {
            formattedDate = date.toString(DateFormatsList.Parser_Date)
        }
        else if let date = serverDate.toDate(DateFormatsList.Server_Date_Time) {
            formattedDate = date.toString(DateFormatsList.Parser_Date)
        }else{
            let splitDate = serverDate.split(separator: "T")
            if splitDate.count > 0 {
                if let date = serverDate.toDate(DateFormatsList.Server_Date) {
                    formattedDate = date.toString(DateFormatsList.Parser_Date)
                }
            }
        }
    ...
    private func convertTime(_ serverTime:String) -> String{
        var formattedTime = serverTime
        if let date = serverTime.toDate("HH:mm:ss") {
            formattedTime = date.toString(DateFormatsList.Parser_Time)
        }else if let date = serverTime.toDate("HH:mm:ss.SSSZ") {
            formattedTime = date.toString(DateFormatsList.Parser_Time)
        }else if let date = serverTime.toDate("HH:mm:ss.SSS") {
            formattedTime = date.toString(DateFormatsList.Parser_Time)
        }
        return formattedTime
    }
    
    private func convertDateTime(_ serverDate:String) -> String{
        var formattedDate = serverDate
        if let date = serverDate.toDate("yyyy-MM-dd'T'HH:mm:ss") {
            formattedDate = date.toString(DateFormatsList.Parser_Date_Time)
        }else if let date = serverDate.toDate("yyyy-MM-dd'T'HH:mm:ss.SSS") {
            formattedDate = date.toString(DateFormatsList.Parser_Date_Time)
        }
        else if let date = serverDate.toDate(DateFormatsList.Server_Date_Time) {
    ...
    func isContainsKey(_ key:String) -> Bool {
        return self.isContainsKey(jsonObject: self, key: key)
    }
    
    func getValue(_ key:String) -> String {
        return self.getString(key, self)
    }
    
    func getDateValue(_ key:String) -> String {
        return convertDate(getValue(key))
    }
    
    func getDateTimeValue(_ key:String) -> String {
        return convertDateTime(getValue(key))
    }
    
    func getTimeValue(_ key:String) -> String {
        return convertTime(getValue(key))
    }
